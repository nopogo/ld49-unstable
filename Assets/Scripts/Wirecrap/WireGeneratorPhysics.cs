...
public class WireGeneratorPhysics : MonoBehaviour {
	...
	void Start()
	{
		...
		for (int i = 0; i < numHosePoints; i++)
		{
			...
			hosePointsLocal[i] = target.InverseTransformPoint(hosePointWorld);
		}
	...
	void Follow()
	{
		...
		float hoseDstBetweenBodyAndHead = Vector3.Distance(hosePath.transform.position, target.position);
	...
	void Hose()
	{
		...
		for (int i = 0; i < numHosePoints; i++)
		{
			hosePointsWorld[i] = target.TransformPoint(hosePointsLocal[i]);
		}

		ikSolver.w1 = w1;
		ikSolver.w2 = w2;
		ikSolver.useDirBias = true;
		ikSolver.endBiasDir = target.up;
		ikSolver.startBiasDir = hosePath.transform.up;
		ikSolver.Solve(hosePointsWorld, hosePath.transform.position);

		for (int i = 0; i < numHosePoints; i++)
		{
			hosePointsLocal[i] = target.InverseTransformPoint(hosePointsWorld[i]);
		}
	...
	void OnValidate()
	{
		if (hosePath != null)
		{
			...
			hosePath.bezierPath.SetPoint(hosePath.bezierPath.NumPoints - 1, hosePath.transform.InverseTransformPoint(target.position));
		}
	}
